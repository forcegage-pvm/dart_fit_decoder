...
class FitFile {
  ...
  List<FitDataMessage> getRecordMessages() {
    return dataMessages
        .where((msg) => msg.globalMessageNumber == FitMessageType.record)
        .toList();
  }
  ...
  List<FitDataMessage> getLapMessages() {
    return dataMessages
        .where((msg) => msg.globalMessageNumber == FitMessageType.lap)
        .toList();
  }
  ...
  List<FitDataMessage> getSessionMessages() {
    return dataMessages
        .where((msg) => msg.globalMessageNumber == FitMessageType.session)
        .toList();
  }
  ...
  List<FitDataMessage> getMessagesByType(int globalMessageNumber) {
    return dataMessages
        .where((msg) => msg.globalMessageNumber == globalMessageNumber)
        .toList();
  }
  ...
  List<FitDataMessage> getMessagesByName(String name) {
    return dataMessages
        .where((msg) => msg.messageName == name)
        .toList();
  }
