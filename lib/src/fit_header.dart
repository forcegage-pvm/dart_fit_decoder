  ...
  factory FitHeader.parse(Uint8List bytes) {
    if (bytes.length < 12) {
      throw InvalidHeaderException(
        'Header too short: ${bytes.length} bytes (minimum 12 required)'
      );
    }
    ...
    if (headerSize != 14 && headerSize != 12) {
      throw InvalidHeaderException(
        'Invalid header size: $headerSize (expected 12 or 14)'
      );
    }
    ...
    if (dataType != '.FIT') {
      throw InvalidHeaderException(
        'Invalid data type signature: "$dataType" (expected ".FIT")'
      );
    }
